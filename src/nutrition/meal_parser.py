...
def _parse_item(raw: str, db, fuzzy: bool = True) -> dict:
    raw = raw.strip()
    m = _GRAMS.match(raw)
    if m:
        grams, name = _num(m.group(1)), m.group(2)
        hit = db.match(name, fuzzy=fuzzy)
        if hit and "per100" in hit:
            return {"raw": raw, "food": hit["name"], "grams": grams, "recognized": True,
                    "source": hit.get("source", "taco"), **_macros(hit["per100"], grams)}
        if hit and "per_portion" in hit:
            qty = grams / hit["portion_g"]
            return {"raw": raw, "food": hit["name"], "grams": grams, "recognized": True,
                    "source": hit.get("source", "manual"), **_from_portion(hit, qty)}
    ...
    if u:
        ...
        hit = db.match(name_clean, fuzzy=fuzzy)
        if hit and "per_portion" in hit:
            return {"raw": raw, "food": hit["name"], "grams": qty * hit["portion_g"],
                    "recognized": True, "source": hit.get("source", "manual"),
                    **_from_portion(hit, qty)}
        pg = db.portion_grams(name)
        hit2 = db.match(name, fuzzy=fuzzy)
        if pg and hit2 and "per100" in hit2:
            grams = qty * pg
            return {"raw": raw, "food": hit2["name"], "grams": grams, "recognized": True,
                    "source": hit2.get("source", "taco"), **_macros(hit2["per100"], grams)}
        return {"raw": raw, "name": name, "recognized": False}
    return {"raw": raw, "recognized": False}


def parse_meal(text: str, db, fuzzy: bool = True) -> dict:
    text = (text or "").strip()
    meal, text = _extract_meal(text)
    parts = [p for p in re.split(r"[,\n]", text) if p.strip()]
    items = [_parse_item(p, db, fuzzy=fuzzy) for p in parts] or [{"raw": text, "recognized": False}]
